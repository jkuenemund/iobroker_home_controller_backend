...
function buildDummyStateIds(deviceSlug, capability) {
	...
	if (capability.type === "blind_position") {
		...
		if (capability.stop) {
			blindIds.stop = `${root}_stop`;
		}
		if (capability.up) {
			blindIds.up = `${root}_up`;
		}
		if (capability.down) {
			blindIds.down = `${root}_down`;
		}
		if (capability.position_feedback) {
			blindIds.position_feedback = `${root}_feedback`;
		}
		return blindIds;
	}
	...
	if (capability.stop) {
		ids.stop = `${base}.${suffix}_stop`;
	}
	if (capability.up) {
		ids.up = `${base}.${suffix}_up`;
	}
	if (capability.down) {
		ids.down = `${base}.${suffix}_down`;
	}
	if (capability.position_feedback) {
		ids.position_feedback = `${base}.${suffix}_feedback`;
	}
	return ids;
}
...
function getDummyCommon(capability, key) {
	...
	switch (capability.type) {
		...
		case "blind_position":
			...
			if (key === "stop") {
				return { ...common, role: "button.stop", type: "boolean" };
			}
			if (key === "up") {
				return { ...common, role: "button.up", type: "boolean" };
			}
			if (key === "down") {
				return { ...common, role: "button.down", type: "boolean" };
			}
...
const RAW_ROOM_DEFINITIONS = {
	office: {
		...
		metrics: [
			{
				type: "temperature",
				state: "hm-rpc.1.000C9A49A31B6B.1.ACTUAL_TEMPERATURE",
				unit: "°C",
				decimals: 1,
				aggregation: "latest",
			},
			{
				type: "humidity",
				state: "hm-rpc.1.000C9A49A31B6B.1.HUMIDITY",
				unit: "%",
				decimals: 0,
				aggregation: "latest",
			},
	...
	diningroom: {
		...
		metrics: [
			{
				type: "temperature",
				state: "hm-rpc.1.000C9A49A31BFF.1.ACTUAL_TEMPERATURE",
				unit: "°C",
				decimals: 1,
				aggregation: "latest",
			},
			{
				type: "humidity",
				state: "hm-rpc.1.000C9A49A31BFF.1.HUMIDITY",
				unit: "%",
				decimals: 0,
				aggregation: "latest",
			},
	...
	floor_eg: {
		...
		metrics: [
			{
				type: "temperature",
				state: "hm-rpc.1.000C9A49A31C07.1.ACTUAL_TEMPERATURE",
				unit: "°C",
				decimals: 1,
				aggregation: "latest",
			},
			{
				type: "humidity",
				state: "hm-rpc.1.000C9A49A31C07.1.HUMIDITY",
				unit: "%",
				decimals: 0,
				aggregation: "latest",
			},
	...
	floor_og: {
		...
		metrics: [
			{
				type: "temperature",
				state: "hm-rpc.1.000C9A49A31B93.1.ACTUAL_TEMPERATURE",
				unit: "°C",
				decimals: 1,
				aggregation: "latest",
			},
			{
				type: "humidity",
				state: "hm-rpc.1.000C9A49A31B93.1.HUMIDITY",
				unit: "%",
				decimals: 0,
				aggregation: "latest",
			},
	...
	joel: {
		...
		metrics: [
			{
				type: "temperature",
				state: "hm-rpc.1.000C9A49A31B7E.1.ACTUAL_TEMPERATURE",
				unit: "°C",
				decimals: 1,
				aggregation: "latest",
			},
			{
				type: "humidity",
				state: "hm-rpc.1.000C9A49A31B7E.1.HUMIDITY",
				unit: "%",
				decimals: 0,
				aggregation: "latest",
			},
	...
	secondChild: {
		...
		metrics: [
			{
				type: "temperature",
				state: "hm-rpc.1.000C9A49A31B5F.1.ACTUAL_TEMPERATURE",
				unit: "°C",
				decimals: 1,
				aggregation: "latest",
			},
			{
				type: "humidity",
				state: "hm-rpc.1.000C9A49A31B5F.1.HUMIDITY",
				unit: "%",
				decimals: 0,
				aggregation: "latest",
			},
	...
	bedroom: {
		...
		metrics: [
			{
				type: "temperature",
				state: "hm-rpc.1.000C9A49A31B7A.1.ACTUAL_TEMPERATURE",
				unit: "°C",
				decimals: 1,
				aggregation: "latest",
			},
			{
				type: "humidity",
				state: "hm-rpc.1.000C9A49A31B7A.1.HUMIDITY",
				unit: "%",
				decimals: 0,
				aggregation: "latest",
			},
	...
	kitchen: {
		...
		metrics: [
			{
				type: "temperature",
				state: "hm-rpc.1.000C9A49A31B67.1.ACTUAL_TEMPERATURE",
				unit: "°C",
				decimals: 1,
				aggregation: "latest",
			},
			{
				type: "humidity",
				state: "hm-rpc.1.000C9A49A31B67.1.HUMIDITY",
				unit: "%",
				decimals: 0,
				aggregation: "latest",
			},
	...
	bathroom_eg: {
		...
		metrics: [
			{
				type: "temperature",
				state: "hm-rpc.1.000C9A49A31B5D.1.ACTUAL_TEMPERATURE",
				unit: "°C",
				decimals: 1,
				aggregation: "latest",
			},
			{
				type: "humidity",
				state: "hm-rpc.1.000C9A49A31B5D.1.HUMIDITY",
				unit: "%",
				decimals: 0,
				aggregation: "latest",
			},
	...
	bathroom_og: {
		...
		metrics: [
			{
				type: "temperature",
				state: "hm-rpc.1.000C9A49A7DE31.1.ACTUAL_TEMPERATURE",
				unit: "°C",
				decimals: 1,
				aggregation: "latest",
			},
			{
				type: "humidity",
				state: "hm-rpc.1.000C9A49A7DE31.1.HUMIDITY",
				unit: "%",
				decimals: 0,
				aggregation: "latest",
			},
	...
	multi_function_room: {
		...
		metrics: [
			{
				type: "temperature",
				state: "hm-rpc.1.000C9A49A31B69.1.ACTUAL_TEMPERATURE",
				unit: "°C",
				decimals: 1,
				aggregation: "latest",
			},
			{
				type: "humidity",
				state: "hm-rpc.1.000C9A49A31B69.1.HUMIDITY",
				unit: "%",
				decimals: 0,
				aggregation: "latest",
			},
...
const VALID_CAPABILITIES = {
	...
	color_temperature: {
		...
		range: [2700, 6500],
		required_state: true,
		description: "Farbtemperatur (2700K-6500K)",
	},
	...
	blind_position: {
		...
		range: [0, 100],
		required_state: true,
		description: "Rollladenposition (0=zu, 100=offen)",
	},
	...
	trigger: {
		type: "boolean",
		required_state: true,
		description: "Trigger-Aktion (setzt auf true, Device setzt automatisch zurück)",
	},
...
const VALID_META_CAPABILITIES = {
	device_available: {
		type: "boolean",
		description: "Gerät online/offline Status (kann mit inverted Parameter invertiert werden)",
	},
		...
		if (!Array.isArray(roomConfig.metrics)) {
			...
			roomConfig.metrics.forEach((m, idx) => {
				...
				if (m.aggregation !== undefined && m.aggregation !== "latest") {
					warnings.push(
						`metrics[${idx}].aggregation unsupported value "${m.aggregation}" (supported: 'latest')`,
					);
				}
			});
		}
	...
	if (roomConfig && Array.isArray(roomConfig.metrics)) {
		metrics = roomConfig.metrics
			...
			.map(m => ({
				...
				aggregation:
					m.aggregation === "latest" || m.aggregation === undefined ? m.aggregation || "latest" : "latest",
			}));
	}
...
function validateDeviceConfig(deviceId, config) {
	...
	if (config.capabilities && Array.isArray(config.capabilities)) {
		config.capabilities.forEach((cap, index) => {
			...
			if (capDef && capDef.range && cap.min_value !== undefined && cap.max_value !== undefined) {
				if (cap.min_value < capDef.range[0] || cap.max_value > capDef.range[1]) {
					warnings.push(
						`capabilities[${index}]: range [${cap.min_value}-${cap.max_value}] exceeds recommended [${capDef.range[0]}-${capDef.range[1]}]`,
					);
				}
			}
		...
		const hasDeviceAvailable = config.capabilities.some(cap => cap.type === "device_available");
		const hasSignalStrength = config.capabilities.some(
			cap => cap.type === "signal_strength" || cap.type === "wifi_signal_strength",
		);

		if (!hasDeviceAvailable) {
			warnings.push(
				"Consider adding device_available capability (use inverted: true for Homematic UNREACH states)",
			);
		}
...
function createDeviceDataPoint(deviceId, config) {
	...
	if (config.capabilities && Array.isArray(config.capabilities)) {
		const capabilityNames = config.capabilities.map(cap => {
			...
			const rangeInfo =
				cap.min_value !== undefined && cap.max_value !== undefined
